...
(function () {
  ...
  if (crumb && mastheadWrap) {
    var LEADER_PAGES = ["a1-who-can-see-what.html", "a2-roles-and-attributes.html", "a3-least-privilege.html", "a4-classify-then-mask.html", "a5-proof-not-promises.html", "a6-operating-model.html"];
    var BUILDER_PAGES = ["b1-grants-first-principles.html", "b2-role-hierarchy.html", "b3-row-level-security.html", "b4-column-security-masking.html", "b5-tokenization.html", "b6-secrets-and-keys.html", "b7-access-review-pipeline.html", "b8-policy-bench.html"];
    var ml = crumb.textContent.match(/Leader session (\d+) of 6/);
    var mb = crumb.textContent.match(/Builder session (\d+) of 8/);
    var m = ml || mb;
    if (m) {
      var current = parseInt(m[1], 10);
      var pages = ml ? LEADER_PAGES : BUILDER_PAGES;
      var letter = ml ? "Leader session " : "Builder session ";
      var journey = document.createElement("div");
      journey.className = "journey";
      var jl = document.createElement("span");
      jl.className = "jlabel";
      jl.textContent = ml ? "Leader track" : "Builder track";
...
(function () {
  "use strict";
  var PASSPORT_KEY = "lwp-passport:data-access-control";
  var TOTAL_SESSIONS = 14;
  var SESSION_KEYS = ["a1", "a2", "a3", "a4", "a5", "a6", "b1", "b2", "b3", "b4", "b5", "b6", "b7", "b8"];
  var SESSION_LABELS = ["L1", "L2", "L3", "L4", "L5", "L6", "B1", "B2", "B3", "B4", "B5", "B6", "B7", "B8"];
  ...
  var cmB = crumbEl && crumbEl.textContent.match(/Builder session (\d+) of 8/);
  var here = cmA ? "a" + cmA[1] : (cmB ? "b" + cmB[1] : null);
  var hereLabel = cmA ? "Leader session " + cmA[1] : (cmB ? "Builder session " + cmB[1] : "");
  ...
  if (strip) {
    ...
    SESSION_KEYS.forEach(function (key, i) {
      ...
      a.title = (got2[key] ? "Stamped: " : "Not stamped yet: ") +
        (key.charAt(0) === "a" ? "leader session " : "builder session ") + key.slice(1);
      strip.appendChild(a);
    });
